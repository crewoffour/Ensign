...
| 7 | Amplifiers and modifiers. 7a complete: HQ staff, task force bracket, installation bar, feint/dummy caret, all 14 echelons, all 13 mobility indicators, ported coordinate-exact from milsymbol's modifier stage and oracle-verified 61/61; render key v2 covering amplifiers; diff.js worklist filter. Discovered en route: milsymbol draws echelons as pure geometry (no text), and its mobility code table (getmetadata.js) is authoritative over 2525D reconstructions. Remaining: 7b exercise glyphs and joker/faker frame remap; 7c control measures assessment and direction-of-movement work in Beacon. | In progress |
| 8+ | Expanded symbol coverage; JMSML migration (incl. adjudicating milsymbol-vs-2525D mapping divergences); METOC scheme | Planned |
